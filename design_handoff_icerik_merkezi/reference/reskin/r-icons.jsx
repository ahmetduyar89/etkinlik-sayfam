...
function CatIcon({ glyph, size = 18, stroke = 'currentColor', sw = 1.7 }) {
  ...
  switch (glyph) {
    case 'sim': // simülasyon — yörünge
      return <svg {...c}><circle cx="12" cy="12" r="3.2" /><ellipse cx="12" cy="12" rx="9.5" ry="4" /><ellipse cx="12" cy="12" rx="9.5" ry="4" transform="rotate(60 12 12)" /><ellipse cx="12" cy="12" rx="9.5" ry="4" transform="rotate(120 12 12)" /></svg>;
...
function RThumb({ act, h = '100%' }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: h, background: `linear-gradient(140deg, ${c}26, ${c}0d 65%, #ffffff00)`, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ color: c, opacity: 0.92 }}><CatIcon glyph={glyph} size={46} stroke={c} sw={1.5} /></div>
      <span style={{ position: 'absolute', bottom: 9, right: 12, fontSize: 9.5, fontWeight: 800, letterSpacing: 1, textTransform: 'uppercase', color: c, opacity: 0.62 }}>{act.category || 'İçerik'}</span>
    </div>
  );
}
